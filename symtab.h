...
/* Definici�n de los tipos posibles de variables */
typedef enum {
    T_ENTERO,
    T_REAL,
    T_BOOLEANO,
    T_CADENA,
    T_ERROR, // Para cuando algo va mal
    T_PLANTILLA // Para definir structs
} tipo_variable;

/* La estructura que guardaremos en la tabla para cada variable */
typedef struct {
    char *nombre;      // El nombre de la variable (lexema)
    tipo_variable tipo; // El tipo (int, float...)
    
    // Usamos una union para ahorrar espacio (como en Bison)
    union {
        int valor_int;      // Para enteros y booleanos (0/1)
        float valor_float;  // Para reales
        char *valor_str;    // Para cadenas
    } u;

    /* Campos para cuando tipo == T_PLANTILLA */
    char* campos[MAX_CAMPOS];
    int tipos_campos[MAX_CAMPOS];
    int num_campos;
} info_simbolo;

/* El puntero a esta estructura es lo que manejar� la symtab */
typedef info_simbolo * sym_value_type;
...
/* esto activa las funciones sym_push_scope() y sym_pop_scope() */
...
#endif
#endif
/* Funci�n auxiliar para instanciar structs */
void sym_instanciar_struct(const char* var_name);
